export const BANK_MODULE = [
  {
    productBankBic: 'HAABAT2K',
    data: {
      imageSrc: 'https://via.placeholder.com/120x53?logo=csm_Anadi_Logo_192d674e89.png',
      link: '/#anadi',
      productBankCountry: 'Österreich',
    }
  },{
    productBankBic: 'BUCUROBU',
    data: {
      imageSrc: 'https://via.placeholder.com/120x53?logo=logo_alpha_bank_160x34.png',
      link: '/#alpha',
      productBankCountry: 'Rumänien',
    }
  },{
    productBankBic: 'ATMBGB22',
    data: {
      imageSrc: 'https://via.placeholder.com/120x53?logo=banklogo/atombank_logo.png',
      link: '/#atom',
      productBankCountry: 'Großbritannien',
      showTooltip: 'Einlagen sind pro Kunde bis 85.000 GBP zu 100 % abgesichert.',
    }
  },{
    productBankBic: 'CBRLGB2L',
    data: {
      imageSrc: 'https://via.placeholder.com/120x53?logo=Close_Brothers_Savings_Logo.png',
      link: '/#closebrothers',
      productBankCountry: 'Großbritannien',
      showTooltip: 'Einlagen sind pro Kunde bis 85.000 GBP zu 100 % abgesichert.',
    }
  },{
    productBankBic: 'PARXLV22',
    data: {
      imageSrc: 'https://via.placeholder.com/120x53?logo=Citadele_Logo_klein.jpg',
      link: '/#cbl',
      productBankCountry: 'Lettland',
    }
  },{
    productBankBic: 'CPLUDES1XXX',
    data: {
      imageSrc: 'https://via.placeholder.com/120x53?logo=CP_Logo_transp_v2.png',
      link: '/#creditplus',
      productBankCountry: 'Deutschland',
    }
  },{
    productBankBic: 'FIMBMTM3XXX',
    data: {
      imageSrc: 'https://via.placeholder.com/120x53?logo=csm_fimbank_730c9feb99.png',
      link: '/#fim',
      productBankCountry: 'Malta',
    }
  },{
    productBankBic: 'BACCFR22',
    data: {
      imageSrc: 'https://via.placeholder.com/120x53?logo=oney_logo_klein.jpg',
      link: '/#oney',
      productBankCountry: 'Frankreich',
    }
  },{
    productBankBic: 'RTMBLV2X',
    data: {
      imageSrc: 'https://via.placeholder.com/120x53?logo=RietumuLogo.gif',
      link: '/#rietumu',
      productBankCountry: 'Lettland',
    }
  },{
    productBankBic: 'BDIGPTPL',
    data: {
      imageSrc: 'https://via.placeholder.com/120x53?logo=big_logo.png',
      link: '/#bigbank',
      productBankCountry: 'Portugal',
    }
  },
]
